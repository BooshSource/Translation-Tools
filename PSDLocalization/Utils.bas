...
Sub replaceToChinesePunctuation(text As String) As String
	text=text.Replace(",","，")
	text=text.Replace(".","。")
	text=text.Replace("!","！")
	text=text.Replace("?","？")
...
Sub replaceToEnglishPunctuation(text As String) As String
	text=text.Replace("，",",")
	text=text.Replace("。",".")
	text=text.Replace("！","!")
	text=text.Replace("？","?")
...
Sub LoadFilesToParents(imageNames As List) As Map
	Dim parents As Map
	parents.Initialize
	For Each name As String In imageNames
		Dim parent As String=GetParentWithoutSlashes(name)
		If parent="" Then
			parent="root"
		End If
		Dim files As List
